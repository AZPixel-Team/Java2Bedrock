...
files = glob.glob("assets/**/sounds.json")
                ...
                for sound in sounds:
                    try:
                        if type(sound).__name__  == "dict":
                            ...
                            shutil.copyfile(f"assets/minecraft/{sound['name'].split(':')[-1]}.ogg", f"staging/target/rp/{sound['name'].split(':')[-1]}.ogg")
                            listsound.append(sound)
                        else:
                            b = (sound.split(":")[-1] + ".ogg").split((sound.split(':')[-1] + ".ogg").split("/")[-1])[0]
                            os.makedirs("staging/target/rp/sounds/" + b, exist_ok=True)
                            shutil.copyfile(f"assets/minecraft/sounds/{sound.split(':')[-1]}.ogg", f"staging/target/rp/sounds/{sound.split(':')[-1]}.ogg")
